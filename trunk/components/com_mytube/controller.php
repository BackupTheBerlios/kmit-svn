...
class mytubeController extends JController
{
    ...
    function __construct()
    {
        ...
        if ($taskName != "rate_video") {//IGNORE AJAX FUNCTION
            ...
            $tabs_toolbar = "<div id=\"glowtabs-container\" style=\"margin-left:" . $c->tabs_margin_left . "px\">
    		    <div id=\"glowingtabs\">
    		    	<ul>";
    ...
    function rate_video()
    {
        ...
        if (count($video) > 0)
        {
            ...
            $ajaxReturn .= "<span class=\"mytube_ratingStars\" title=\"".round($video->ratingAvg, 1) ." out of 5 stars\">";
            ...
            {
                $ajaxReturn .= "<br /><span id=\"ratingThanks\" class=\"mytube_ratingThanks\">Thanks for Voting!</span> <span class=\"mytube_ratingCount\">1 rating</span>";
            }
            else if ($video->ratingCount == 0)
            {
                $ajaxReturn .= "<br /><span id=\"ratingThanks\" class=\"mytube_ratingThanks\">Thanks for Voting!</span> <span class=\"mytube_ratingCount\">Not yet rated</span>";
            }
            else
            {
                $ajaxReturn .= "<br /><span id=\"ratingThanks\" class=\"mytube_ratingThanks\">Thanks for Voting!</span> <span class=\"mytube_ratingCount\">".$video->ratingAvg." ratings</span>";
            }
    ...
    function applylink() {
        ...
        $vidlink=mytube_urldecode($link);
        $current_dir = JPATH_ADMINISTRATOR.'/components/com_mytube/plugins/videoserver/';
    ...
    function uploadvideo() {
        ...
        if ($c->uploader_type == "flashupload") {
            ...
            '../administrator/components/com_mytube/assets/lib/FlashUploader/',           // a base path to Flash Uploader's directory (relative to the page)
            'upload.php',       // path to a file that handles file uploads (relative to uploader.swf) [optional]
            'administrator/components/com_mytube/assets/lib/FlashUploader/config.php'  // path to a server-side config file (relative to the page) [optional]
            );
...
//                echo '</pre>';
        ...
        if ($returnid = $model->store($post, 0)) {
            $msg = JText::_( 'VIDEO SAVED' );
            $link 	= JRoute::_('index.php?option=com_mytube&view=video&id=' . $returnid);
            $cache = &JFactory::getCache('com_mytube');
            $cache->clean();
        } else {
            $msg 	= '';
