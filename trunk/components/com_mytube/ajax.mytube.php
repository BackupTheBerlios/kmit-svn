...
//Require helperfile
require_once (JPATH_BASE . DS . 'components'.DS.'com_mytube' . DS . 'helpers' . DS . 'helpers.php');
require_once (JPATH_BASE . DS . 'components'.DS.'com_mytube' . DS . 'assets' . DS . 'lib' . DS . 'mix.php');

// include language file
$lang_file = JPATH_BASE . DS . 'components'.DS.'com_mytube' . DS . 'languages' . DS . $c->jtube_language . '.php';
$lang_default_file = JPATH_BASE . DS . 'components'.DS.'com_mytube' . DS . 'languages' . DS . 'english.php';
...
function aio() {
    ...
    <?php foreach ($videos as $video):?>
							...
							<div class="v90WideEntry">
								<div class="v90WrapperOuter">
									...
									<?php if ($video->video_type == 'local' || $video->video_type == ''):?>
					   				<a href="index.php?option=com_mytube&view=video&id=<?php echo $video->id; ?>" id="video-url-e8FGIveLwyw">
						...
						<div class="video-main-content">
							<div class="video-mini-title"><a href="index.php?option=com_mytube&view=video&id=<?php echo $video->id; ?>" title="<?php echo $video->video_title?>"><?php echo stripslashes($video->video_title);?></a>
							</div>
							<div class="video-view-count"><?php echo $video->hits?> <?php echo strtolower(_VIDEOINFO_NUMBER_VIEWS) ?>
							</div>
							<div class="video-username"><?php echo _VIDEOINFO_AUTHOR ?>: <a href="index.php?option=com_mytube&view=videos&type=member&user_id=<?php echo $video->user_id?>"><?php echo mytube::showShortAuthor($video->username)?></a>
	  						</div>
						</div>
					</div>
					...
					<?php endforeach;?>
				  					<?php if ($count > 20):?>
				  					<center><a href="index.php?option=com_mytube&view=videos&category_id=<?php echo $catid?>">See all <?php echo $count?> videos</a></center>
				  					<?php endif;?>
