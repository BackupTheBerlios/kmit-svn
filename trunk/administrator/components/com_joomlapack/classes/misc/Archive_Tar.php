...
// Modified for JoomlaPack by Nicholas K. Dionysopoulos

// Check to ensure this file is within the rest of the framework
defined('JPATH_BASE') or die();

// Fix 2.3.3 -- Do not reload PEAR if it is already loaded in a non Joomla!-friendly
// way, e.g. by a funky host setup, or a misbehaving extension... 
if(!class_exists('PEAR')) { jimport('pear.PEAR'); }
...
class Archive_Tar extends PEAR
{
	...
	/**
	 * @var JoomlapackPackerTAR Owning object
	 */
	var $owningObject;
	...
	* @see addModify()
	*/
	function createModify($p_filelist, $p_add_dir, $p_remove_dir='', $p_saveas)
	{
		$v_result = true;
		...
		if ($p_filelist != '') {
			...
			{
				$v_list = $p_filelist;
				$v_saveaslist = $p_saveas;
			}
			elseif (is_string($p_filelist))
			{
				$v_list = explode($this->_separator, $p_filelist);
				$v_saveaslist = explode($this->_separator, $p_saveas);
			}
			else {
				$this->_cleanFile();
			...
			$v_result = $this->_addList($v_list, $p_add_dir, $p_remove_dir, $v_saveaslist);
		}
	...
	* @access public
	*/
	function addModify($p_filelist, $p_add_dir, $p_remove_dir='', $p_saveas)
	{
		$v_result = true;

		if (!@is_file($this->_tarname))
		$v_result = $this->createModify($p_filelist, $p_add_dir, $p_remove_dir, $p_saveas);
		else {
			if (is_array($p_filelist))
			{
				$v_list = $p_filelist;
			...
			{
				$v_list = explode($this->_separator, $p_filelist);
				$v_saveas = explode($this->_separator, $p_saveas);
			}
			else {
				$this->_error('Invalid file list');
				return false;
			...
			$v_result = $this->_append($v_list, $p_add_dir, $p_remove_dir, $v_saveas);
		}
	...
	*/
	...
	function _writeBlock($p_binary_data, $p_len=null)
	{
		if (is_resource($this->_file)) {
			if ($p_len === null) {
				...
				$this->owningObject->_fwrite($this->_file, $p_binary_data);
	...
	// {{{ _addList()
	function _addList($p_list, $p_add_dir, $p_remove_dir, $p_saveaslist)
	{
		$v_result=true;
		$v_header = array();
		...
		for ($j=0; ($j<count($p_list)) && ($v_result); $j++) {
			...
			if (!$this->_addFile($v_filename, $v_header, $p_add_dir, $p_remove_dir, $v_saveas))
			return false;
	...
	// {{{ _addFile()
	function _addFile($p_filename, &$p_header, $p_add_dir, $p_remove_dir, $p_saveas)
	{
		if (!$this->_file) {
			$this->_error('Invalid file descriptor');
			return false;
		...
		$p_saveas = $this->_translateWinPath($p_saveas, false);;
		$v_stored_filename = $p_saveas;
		...
		if ($p_remove_dir != '') {
			...
			if (substr($p_saveas, 0, strlen($p_remove_dir)) == $p_remove_dir)
			$v_stored_filename = substr($p_saveas, strlen($p_remove_dir));
		}
	...
	*/
	...
	// {{{ _append()
	function _append($p_filelist, $p_add_dir='', $p_remove_dir='', $p_saveas)
	{
		if (!$this->_openAppend())
		return false;

		if ($this->_addList($p_filelist, $p_add_dir, $p_remove_dir, $p_saveas))
		$this->_writeFooter();
