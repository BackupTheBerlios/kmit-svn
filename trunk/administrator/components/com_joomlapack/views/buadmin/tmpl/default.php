...
<form action="<?php echo JURI::base(); ?>index.php" method="post"
	...
	id="option" value="com_joomlapack" /> <input type="hidden" name="view"
	id="view" value="buadmin" /> <input type="hidden" name="boxchecked"
...
<table class="adminlist">
	...
	if($record->meta == 'ok')
	{
		...
		$filename_col = '';
		$link_target = JURI::base().'/index.php?option=com_joomlapack&view=buadmin&task=download&id='.urlencode($record->id);
		if($record->multipart == 0)
		{
			// Single part file -- Create a simple link
			$filename_col = "<a href=\"$link_target\">".$record->archivename."</a>";
		}
		else
		{
			...
			{
				$link_target = JURI::base().'/index.php?option=com_joomlapack&view=buadmin&task=download&id='.urlencode($record->id).'&part='.urlencode($count);
				$label = JText::sprintf('STATS_LABEL_PART', $count);
				$filename_col .= ($count > 0) ? ' &bull; ' : '';
				$filename_col .= "<a href=\"$link_target\">$label</a>";
			}
		}
	}
	else
	...
	?>
		...
		<?php if($this->easy): ?>
			...
			<?php else: ?>
			<td><?php echo $check; ?></td>
			<td><?php echo $info_link ?><?php echo $record->description ?></td>
			<td><?php echo $startTime->toFormat(JText::_('DATE_FORMAT_LC4')); ?></td>
