...
<form action="index.php" method="post" name="adminForm">
	...
	<tbody>
		...
		for ($i=0, $n=count($this->rows); $i < $n; $i++) {
			...
			$link 		= 'index.php?option=com_mytube&amp;controller=videos&amp;task=edit&amp;cid[]='. $row->id;
        ...
        <?php echo trim($row->video_type) == '' ? 'Local' : ucfirst($row->video_type)?>
			...
			<td align="center">
					...
					if ($row->featured == 1) {
					?>
						<img style="cursor: pointer;" onclick="location.href='index.php?option=com_mytube&task=unfeatured&controller=videos&id=<?php echo $row->id?>'" src="images/tick.png" width="16" height="16" border="0"/>
					<?php
					}else {
					?>
						<img style="cursor: pointer;" onclick="location.href='index.php?option=com_mytube&task=featured&controller=videos&id=<?php echo $row->id?>'" src="images/publish_x.png" width="16" height="16" border="0"/>
					<?php
					}
					?>
				</td>
