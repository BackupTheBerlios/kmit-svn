...
class InstallerJomTubePlugin extends JObject
{
	...
	function install()
	{
		...
		if (!empty ($pname) && !empty($group)) {
		    switch ($group) {
		        case 'thirdparty':
		            $this->parent->setPath('extension_root', JPATH_ADMINISTRATOR.DS.'components'.DS.'com_mytube'.DS.'plugins'.DS.'videoserver');
		            $folder = "administrator/components/com_mytube/plugins/videoserver";
		            break;
		        case 'template':
		            $this->parent->setPath('extension_root', JPATH_COMPONENT_SITE.DS.'assets'.DS.'styles'.DS.$pname);
		            $folder = "components/com_mytube/assets/styles/".$pname;
		            break;
		    }
